...
parser.add_argument('--SUN-dir', type=str, default='./data/SUN/SUN397',
                        help='path to dataset')
parser.add_argument('--covid-dir', type=str, default='./data/COVID_images')

parser.add_argument('--train-COV', type=str, default='./data/COVID_images/trainCT_COVID.txt')
parser.add_argument('--train-NonCOV', type=str, default='./data/COVID_images/trainCT_NonCOVID.txt')
...
parser.add_argument('--save-path',default='SUN+COVID',type=str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    coviddata = CovidCTDataset(root_dir=args.covid_dir,
                              txt_COVID=args.train_COV,
                              txt_NonCOVID=args.train_NonCOV)

    SUNdataset = SUNDataset(root=args.SUN_dir, number_per_class=1000)
    coviddata_img = [path for (path,label) in coviddata.img_list]
    coviddata_label = [label for (path,label) in coviddata.img_list]

    train_dataset = CombineDataset(images1 = coviddata_img,images2 = SUNdataset.images,
                               labels1 = coviddata_label, labels2 = SUNdataset.labels,
