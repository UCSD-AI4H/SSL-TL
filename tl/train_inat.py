...
def train(model, train_loader, optimizer, PRINT_INTERVAL, epoch, args,LOSS_FUNC, device):
    ...
    for index, (images, im_id, target, tax_ids) in enumerate(tqdm(train_loader)):
...
def main():
    ...
    parser.add_argument('--checkpoint-path',type = str, default='./checkpoint/inat/',
    ...
    parser.add_argument('--root-dir', type=str, default='./data/inat/',
                        help='path to the image folder')
    parser.add_argument('--train-json', type=str, default='./data/train2018.json',
                        help='path to the train folder, each class has a single folder')
    parser.add_argument('--val-json', type=str, default='../data/val2018.json',
    ...
    parser.add_argument('--ratio', type=float, default=50,help='Number of image per class')
    ...
    parser.add_argument('--num-class', type = int, default=8142, help='Number of class for the classification')
    ...
    trainset = INAT(root=args.root_dir,ann_file=args.train_json,ratio=args.ratio,
    ...
    valset = INAT(root=args.root_dir,ann_file=args.val_json,transforms=test_trans)
