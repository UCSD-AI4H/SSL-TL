...
def main():
    ...
    parser.add_argument('--checkpoint-path',type = str, default='./tl/combine_cal',
    ...
    parser.add_argument('--caltech-dir', type=str, default='./dataset/Caltech256',
                        help='path to the image folder')
    ...
    parser.add_argument('--resume', type=str, default='./tl/combine_cal/best.pth.tar',
                        help='Path to resume a checkpoint')
    parser.add_argument('--num-class', type = int, default=653, help='Number of class for the classification')
    ...
    Caltech = Caltech256(root=args.caltech_dir, train_file=args.train_file, val_file=args.val_file,
                         test_file=args.test_file, download=True)
    caltechtrain = ImageDataset(images=Caltech.train_images, labels=Caltech.train_label)
    caltechval = ImageDataset(images=Caltech.val_images, labels=Caltech.val_label)
    SUNdataset = SUNDataset(root=args.SUN_dir, number_per_class=1000)
    train_dataset = CombineDataset(images1=caltechtrain.images, images2=SUNdataset.images,
                                   labels1=caltechtrain.labels, labels2=SUNdataset.labels,
                                   transforms=transforms.Compose(augmentation))
    print('train set {}'.format(len(train_dataset)))

    valset = CombineDataset(images1=caltechval.images, images2=SUNdataset.images,
                                   labels1=caltechval.labels, labels2=SUNdataset.labels,
                                   transforms=transforms.Compose(augmentation))
    testset = CombineDataset(images1=caltechval.images, images2=SUNdataset.images,
                            labels1=caltechval.labels, labels2=SUNdataset.labels,
                            transforms=transforms.Compose(augmentation))
