...
def main():
    ...
    parser.add_argument('--checkpoint-path',type= str, default='./checkpoint/s_inat9w_t_flower102_resnet50_tl/',
    ...
    parser.add_argument('--root-dir', type=str, default='../data/Flower102',
    ...
    parser.add_argument('--transfer-resume', type=str, default='./checkpoint/inat_resnet50_tl9w_new/best.pth.tar',
                        help='Path to load transfering pretrained model')
    parser.add_argument('--resume', type = str, default='./checkpoint/inat_resnet50_tl9w_new/best.pth.tar',
                        help='Path to resume a checkpoint')
    parser.add_argument('--num-class', type = int, default=102, help='Number of class for the classification')
    ...
    trainset = Flower102(root_dir=args.root_dir, mode = 'train',
                                    transforms=transforms.Compose(
                                        [transforms.Resize(256),
                                         transforms.RandomResizedCrop((224, 224), scale=(0.5, 1.2)),
    ...
    valset = Flower102(root_dir=args.root_dir, mode = 'val', transforms=test_trans)

    testset = Flower102(root_dir=args.root_dir, mode = 'test', transforms=test_trans)
    ...
    sheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=6)
