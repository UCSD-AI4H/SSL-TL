...
class ImageNetDataset(data.Dataset):
    ...
    def __init__(self, root,number_per_class,number_cls=1000,mode = 'train',transforms=None):
        ...
        for i,c in enumerate(self.category):
            cls_dir = os.path.join(self.root,c)
            num_exist = len(os.listdir(cls_dir))
            number_per_class = min(number_per_class,num_exist)
            if mode=='train':
                cls_list = [os.path.join(cls_dir,item) for item in os.listdir(cls_dir)][:number_per_class]
            else:
                cls_list = [os.path.join(cls_dir, item) for item in os.listdir(cls_dir)][250:]
                cls_list = random.sample(cls_list,number_per_class)
            self.images.extend(cls_list)
            self.labels.extend(number_per_class*[i])
        self.transforms = transforms

        print('Number of class {}'.format(len(self.category)))
...
class ImageNetDatasetUnbalance(data.Dataset):
    def __init__(self, root, max_num, min_num, number_cls=1000, mode='train', transforms=None):
        self.number_cls  = number_cls
        self.root = root
        self.category = sorted(os.listdir(root))
        self.images = []
        ...
        self.max = max_num
        self.min = min_num
        self.number_per_cls = np.zeros(number_cls)
        for i in range(self.number_cls):
            self.number_per_cls[i] = int((i/self.number_cls)*(self.max-self.min) + self.min)
        ...
        for i, c in enumerate(self.category):
            cls_dir = os.path.join(self.root, c)
            num_exist = len(os.listdir(cls_dir))
            number_per_class = int(min(self.number_per_cls[i], num_exist))
...
def main():
    ...
    parser.add_argument('--checkpoint-path',type = str, default='./checkpoint/imagnet_resnet50_tl_unbalance1/',
    ...
    parser.add_argument('--root-dir', type=str, default='../../ImageNet-Datasets-Downloader/imagenet/imagenet_images/',
    ...
    parser.add_argument('--schedule', default=[15,30,130], nargs='*', type=int,
                        help='learning rate schedule (when to drop lr by 10x)')

    parser.add_argument('--pretrained',type=str, default='Transfer',
    ...
    parser.add_argument('--PRINT-INTERVAL', type=int, default=50, help='Number of batch to print the loss')
    ...
    trainset = ImageNetDataset(root=args.root_dir,number_per_class=130,transforms=train_trans)
    valset = ImageNetDataset(root=args.root_dir,number_per_class=10,mode='test',transforms=test_trans)
